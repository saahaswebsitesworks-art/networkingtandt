...
// managed live from /admin/rates and stored in Neon Postgres (see
...
// - Airport, outstation & one-way are all tiered: distance up to the
//   minimum km is charged at the base ratePerKm, and any distance beyond
//   that minimum is charged at that section's own extraRatePerKm (set in
//   the admin rates editor). If extraRatePerKm isn't set on a vehicle yet
//   (older data), we fall back to the base ratePerKm so nothing breaks.
...
export function calculatePrice({ vehicles, vehicleId, tripType, km = 0, days = 1, localPackageIdx = 0, gstRate = 0.05 }) {
  ...
  if (tripType === 'airport') {
    ...
    if (v.airport.flat) {
      ...
      const extraRate = v.airport.extraRatePerKm ?? v.local?.extraKmRate;
      if (safeKm > v.airport.minKm && extraRate) {
        const extraKm = safeKm - v.airport.minKm;
        const extraAmt = extraKm * extraRate;
        subtotal += extraAmt;
        breakdown.push({ label: `Extra ${extraKm} km x ₹${extraRate}/km`, amount: extraAmt });
      }
    } else {
      const baseKm = v.airport.minKm;
      const baseAmt = baseKm * v.airport.ratePerKm;
      if (safeKm > baseKm) {
        const extraKm = safeKm - baseKm;
        // Fall back to the base rate for vehicles saved before this field existed.
        const extraRate = v.airport.extraRatePerKm ?? v.airport.ratePerKm;
        const extraAmt = extraKm * extraRate;
        subtotal = baseAmt + extraAmt;
        breakdown.push({ label: `${baseKm} km x ₹${v.airport.ratePerKm}/km (minimum)`, amount: baseAmt });
        breakdown.push({ label: `Extra ${extraKm} km x ₹${extraRate}/km`, amount: extraAmt });
      } else {
        subtotal = baseAmt;
        breakdown.push({ label: `${baseKm} km x ₹${v.airport.ratePerKm}/km (min ${baseKm} km)`, amount: baseAmt });
      }
    }
    ...
    let fare;
    if (safeKm > totalMinKm) {
      const baseAmt = totalMinKm * v.outstation.ratePerKm;
      const extraKm = safeKm - totalMinKm;
      const extraRate = v.outstation.extraRatePerKm ?? v.outstation.ratePerKm;
      const extraAmt = extraKm * extraRate;
      fare = baseAmt + extraAmt;
      breakdown.push({
        label: `${totalMinKm} km x ₹${v.outstation.ratePerKm}/km (minimum for ${dayLabel})`,
        amount: baseAmt,
      });
      breakdown.push({ label: `Extra ${extraKm} km x ₹${extraRate}/km`, amount: extraAmt });
    } else {
      fare = totalMinKm * v.outstation.ratePerKm;
      breakdown.push({
        label: `${totalMinKm} km x ₹${v.outstation.ratePerKm}/km (min ${totalMinKm} km for ${dayLabel})`,
        amount: fare,
      });
    }
    const da = v.outstation.da * safeDays;
    subtotal = fare + da;
    breakdown.push({ label: `Driver Allowance (₹${v.outstation.da} x ${dayLabel})`, amount: da });
    ...
    let fare;
    if (safeKm > v.oneWay.minKm) {
      const baseAmt = v.oneWay.minKm * v.oneWay.ratePerKm;
      const extraKm = safeKm - v.oneWay.minKm;
      const extraRate = v.oneWay.extraRatePerKm ?? v.oneWay.ratePerKm;
      const extraAmt = extraKm * extraRate;
      fare = baseAmt + extraAmt;
      breakdown.push({
        label: `${v.oneWay.minKm} km x ₹${v.oneWay.ratePerKm}/km (minimum)`,
        amount: baseAmt,
      });
      breakdown.push({ label: `Extra ${extraKm} km x ₹${extraRate}/km`, amount: extraAmt });
    } else {
      fare = v.oneWay.minKm * v.oneWay.ratePerKm;
      breakdown.push({
        label: `${v.oneWay.minKm} km x ₹${v.oneWay.ratePerKm}/km (min ${v.oneWay.minKm} km)`,
        amount: fare,
      });
    }
